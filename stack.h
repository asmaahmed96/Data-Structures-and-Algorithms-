...
{ 
 ...
 public:  // available to the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: give variable to recieve the top element in the stack using pass by reference.
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: Check to see if the stack is empty. Returns true if empty and fals e if full.
  bool isEmpty();

  //PURPOSE: Checks to see if the stack is full. Returns true if full and false if empty.
  bool isFull();

  //PURPOSE: Display all the elements in the stack.
  void displayAll();

  //PURPOSE: Empty the stack using a while loop and pop each element.
  void clearIt();
