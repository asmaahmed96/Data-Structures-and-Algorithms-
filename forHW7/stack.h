...
typedef char el_t;     // el_t type is ** for now
...
{ 
 ...
 public:  // available to the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: provide a holder for the top element (pass by ref)
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: checks to see if stack is empty
  bool isEmpty();

  //PURPOSE: checks to see if stack is full
  bool isFull();

  //PURPOSE: displays all elements of the stack without changing the stack
  void displayAll();
