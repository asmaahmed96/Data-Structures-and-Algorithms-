...
void stack::clearIt()
{
   el_t temp;  // local variable (elem) provides holder for the element popped (pass by ref)
   while(!isEmpty()) // loop calls pop function while stack is not empty (do not do top = -1, use a loop)
     {
      pop(temp);
     } 
}
