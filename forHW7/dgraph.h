...
// returns the adjacency list of a given vertex - may throw BadVertexx
  ...
  //gives visit number to be stored in visit and vertex that visit number will go to
  void visit(int,char);
  //checks if visit number is already assigned to vertex, return 0 if already visited
  bool isMarked(char);
};
