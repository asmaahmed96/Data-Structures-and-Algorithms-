...
void BST::DeleteVertex(elem_t E)
{
  ...
  else if(E==Root->Elem &&((Root->Left != NULL && Root->Right==NULL)||(Root->Left==NULL&&Root->Right!=NULL)))// if what you want to delete is the root
  { cout << "... deleting the root with just one child" << endl; 
    ...
    {  
      ...
      delete V;//   making sure the old root has been deleted and the new root's UP is NULL
      Root->Up = NULL;
      return; 
    }
  ...
  while (V != NULL)
	...
	{  cout << "...removing " << V->Elem << endl;
	  remove(V,V->Up); // call remove here to remove V
	  return; }
...
void BST::remove(Vertex *V, Vertex *P)
{
  ...
  if(V->Right == NULL && V->Left == NULL)
	...
	{
	  P->Left = C;// should point to V's left child; 
      ...
      else 
	{
     ...
     { Vertex* C = V->Right;  // the right child
       ...
       if(P->Left == V)//You need if then else to determine Parent's left or right
	 P->Left = C; // should point to V's right child; 
       else 
	 P->Right = C;
...
{
  cout << "Re-caluclating height and balance" << endl;
  while(V != NULL) //special case for when right or left child does not exist
    {
      if(V->Right == NULL || V->Left == NULL)
	{
	  if(V->Right == NULL && V->Left != NULL)
	  ...
	  if(V->Right == NULL && V->Left != NULL)
	    {
	      V->Height = max(V->Left->Height, -1)+1;
	      V->Balance = (-1)-(V->Left->Height);
	    }
	  if(V->Right != NULL && V->Left == NULL)
	    {
	      V->Height = max(V->Right->Height, -1)+1;
	      V->Balance =(V->Right->Height)-(-1);
	    }
	}
      else
	{
	  V->Height = (max(V->Right->Height, V->Left->Height))+1; //find height of left/ right child
	  V->Balance =(V->Left->Height)- (V->Right->Height); //find balance of left/right child
	}
      cout << "..." << V->Elem << " 's Height: " << V->Height << " and Balance: " << V->Balance << endl;
      V = V->Up;
