...
{ 
  ...
  while(!L.isEmpty()) // loop - use x for removed element
    {
      L.deleteRear(x);
      cout << "Deleted: " << x << endl;
    }
  ...
  try {L.deleteRear(x);}
  catch (llist::Underflow){cerr << "ERROR: Underlfow" << endl;}
...
//Algorithm/Design: 3 test cases are divided into functions and each case will implement differnt functions of llist.
//Case 1 will use addRear function to add elements into the list. It will then display what is in the list, also remove the elements using deleteFront and deleteRear.
//case 2 will use addFront function to add elemts from the front of the list and then use insertIth and deleteIth to add and delete element from the list
//case 3 will use the copy constructor and operator overloading function for = to copy one llist to another.
int main()
{
